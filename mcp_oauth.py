"""MCP OAuth2 + Google Login (with JWKS verification)"""
...
class GoogleJWKSValidator:
    ...
    async def verify_id_token(self, id_token: str) -> dict:
        ...
        except ImportError: raise ImportError("python-jose is required: pip install python-jose[cryptography]")
        jwks = await self._fetch_jwks()
        try: hdr = jwt.get_unverified_header(id_token)
        except JWTError as e: raise ValueError(f"Failed to read token header: {e}")
        kid = hdr.get("kid")
        if not kid: raise ValueError("No kid found in token")
        rsa_key = next((k for k in jwks.get("keys", []) if k.get("kid") == kid), None)
        if not rsa_key: raise ValueError(f"No public key found for kid={kid}")
        try:
        ...
        except JWTError as e: raise ValueError(f"Token verification failed: {e}")
        if not payload.get("email_verified"): raise ValueError("Email is not verified")
        return payload
...
class MCPOAuthProvider(OAuthAuthorizationServerProvider):
    def __init__(self, store, base_url, token_ttl=3600, google_client_id="", google_client_secret="", allowed_emails=None):
        ...
        if not google_client_id or not google_client_secret: raise ValueError("google_client_id and google_client_secret are required.")
    ...
    async def complete_google_callback(self, code, state):
        ...
        if not pending: raise ValueError("Invalid or expired state")
        tr = await self._exchange_google_code(code)
        id_token = tr.get("id_token")
        if not id_token: raise ValueError("No id_token received from Google")
        payload = await self.jwks_validator.verify_id_token(id_token)
        user_email = payload.get("email")
        if not user_email: raise ValueError("No email found in token")
        print(f"✔ JWKS verification successful: {user_email} - {payload.get('name','')} - verified:{payload.get('email_verified')}")
        if self.allowed_emails and user_email not in self.allowed_emails:
            raise ValueError(f"User ({user_email}) is not in the allowed list.")
